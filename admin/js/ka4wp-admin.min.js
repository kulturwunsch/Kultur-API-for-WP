...
$(document).ready(function() {
	...
	$('body').on( 'click', '.ka4wp-upload-file', function(event){
		...
		const customUploader = wp.media({
			title: lang.insert_image, // modal window title
			library : {
				// uploadedTo : wp.media.view.settings.post.id, // attach to the current post?
				type : 'image'
			},
			button: {
				text: lang.use_image // button label text
			},
			multiple: false
		}).on( 'select', function() {
	...
	$('body').on( 'click', '.ka4wp-remove-file', function( event ){
		event.preventDefault();
		const button = $(this);
		button.next().val( '' );
		button.addClass('d-none').prev().addClass( 'button' ).html( lang.upload_image );
	});
